...
def select_mixed(price, trend_w=0.7, hurst_thr=0.55, vr_p_thr=0.01,
    ...
    if isinstance(base, (EMACrossStrategy, DonchianStrategy)):
        ...
        return EMACrossStrategy(price) if random.random() < trend_w else DonchianStrategy(price)
    else:
        # MR 系 → BBand vs RSI ((1-trend_w)で重み付け)
        return BBandMRStrategy(price) if random.random() < (1-trend_w) else RSIMRStrategy(price) 